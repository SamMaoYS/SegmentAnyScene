...
class PyrenderRenderer:
    ...
    def set_scene(self, scene, transform=None):
        ...
        self.scene = pyrender.Scene.from_trimesh_scene(
            trimesh_scene, bg_color=self._bg_color
        )
    ...
    def initialize_camera(self, automatic=True):
        ...
        pcam = pyrender.PerspectiveCamera(
            yfov=self.yfov, znear=self.znear, zfar=self.zfar
        )
    ...
    def render(self, window_title="Window"):
        ...
        if self.interactive:
            pyrender.Viewer(
                ...
                render_flags={"point_size": self.point_size},
            )
        else:
